...
switch(format.toLowerCase()){case"raw":return this.subtle.importKey("raw",keyData,algorithm,extractable,keyUsages);case"spki":{const asn1=asn1js.fromBER(keyData);if(asn1.offset===-1)return Promise.reject("Incorrect keyData");const publicKeyInfo=new _PublicKeyInfo.default();try{publicKeyInfo.fromSchema(asn1.result);}catch(ex){return Promise.reject("Incorrect keyData");}// noinspection FallThroughInSwitchStatementJS
...
const publicKeyJSON=publicKeyInfo.toJSON();var _arr=Object.keys(publicKeyJSON);for(var _i=0;_i<_arr.length;_i++){const key=_arr[_i];jwk[key]=publicKeyJSON[key];}//endregion
...
const publicKeyJSON=publicKeyInfo.toJSON();var _arr2=Object.keys(publicKeyJSON);for(var _i2=0;_i2<_arr2.length;_i2++){const key=_arr2[_i2];jwk[key]=publicKeyJSON[key];}//endregion
...
const publicKeyJSON=publicKeyInfo.toJSON();var _arr3=Object.keys(publicKeyJSON);for(var _i3=0;_i3<_arr3.length;_i3++){const key=_arr3[_i3];jwk[key]=publicKeyJSON[key];}//endregion
}break;default:return Promise.reject(`Incorrect algorithm name: ${algorithm.name.toUpperCase()}`);}}break;case"pkcs8":{const privateKeyInfo=new _PrivateKeyInfo.default();//region Parse "PrivateKeyInfo" object
const asn1=asn1js.fromBER(keyData);if(asn1.offset===-1)return Promise.reject("Incorrect keyData");try{privateKeyInfo.fromSchema(asn1.result);}catch(ex){return Promise.reject("Incorrect keyData");}if("parsedKey"in privateKeyInfo===false)return Promise.reject("Incorrect keyData");//endregion
...
const privateKeyJSON=privateKeyInfo.toJSON();var _arr4=Object.keys(privateKeyJSON);for(var _i4=0;_i4<_arr4.length;_i4++){const key=_arr4[_i4];jwk[key]=privateKeyJSON[key];}//endregion
...
const privateKeyJSON=privateKeyInfo.toJSON();var _arr5=Object.keys(privateKeyJSON);for(var _i5=0;_i5<_arr5.length;_i5++){const key=_arr5[_i5];jwk[key]=privateKeyJSON[key];}//endregion
...
//region Create RSA Private Key elements
const privateKeyJSON=privateKeyInfo.toJSON();var _arr6=Object.keys(privateKeyJSON);for(var _i6=0;_i6<_arr6.length;_i6++){const key=_arr6[_i6];jwk[key]=privateKeyJSON[key];}//endregion
...
return Promise.resolve().then(()=>this.subtle.importKey("jwk",(0,_pvutils.stringToArrayBuffer)(JSON.stringify(jwk)),algorithm,extractable,keyUsages)).then(result=>result,()=>this.subtle.importKey("jwk",jwk,algorithm,extractable,keyUsages));}//endregion
...
{try{const params=new _RSASSAPSSParams.default({schema:signatureAlgorithm.algorithmParams});if("hashAlgorithm"in params){const algorithm=this.getAlgorithmByOID(params.hashAlgorithm.algorithmId);if("name"in algorithm===false)return"";result=algorithm.name;}else result="SHA-1";}catch(ex){}}break;default:}return result;}//**********************************************************************************
/**
...
const ivView=new Uint8Array(ivBuffer);this.getRandomValues(ivView);const saltBuffer=new ArrayBuffer(64);const saltView=new Uint8Array(saltBuffer);this.getRandomValues(saltView);const contentView=new Uint8Array(parameters.contentToEncrypt);const pbkdf2Params=new _PBKDF2Params.default({salt:new asn1js.OctetString({valueHex:saltBuffer}),iterationCount:parameters.iterationCount,prf:new _AlgorithmIdentifier.default({algorithmId:hmacOID,algorithmParams:new asn1js.Null()})});//endregion
...
algorithmParams:pbes2Parameters.toSchema()}),encryptedContent:new asn1js.OctetString({valueHex:result})});},error=>Promise.reject(error));//endregion
return sequence;}//**********************************************************************************
/**
...
algorithmParams:paramsObject.hashAlgorithm.toSchema()});}if(parameters.algorithm.saltLength!==20)paramsObject.saltLength=parameters.algorithm.saltLength;const pssParameters=new _RSASSAPSSParams.default(paramsObject);//endregion
